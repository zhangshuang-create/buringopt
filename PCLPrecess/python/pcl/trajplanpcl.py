...
def build_nonconvex_uv_domain(
    uv: np.ndarray, alpha_factor: float = 3.0,
) -> tuple[Delaunay, np.ndarray, float]:
    """Build an alpha-like non-convex domain from a UV point cloud.

    SciPy's Delaunay triangulation fills the convex hull.  Long triangles are
    the bridges across concavities and holes, so discard triangles whose
    longest edge is large relative to the local point spacing.
    """
    if alpha_factor <= 1.0:
        raise ValueError("trajectory alpha factor must be greater than 1")
    triangulation = Delaunay(uv)
    simplices = triangulation.simplices
    if len(simplices) == 0:
        raise ValueError("Cannot triangulate the UV point cloud")

    neighbor_distances, _ = cKDTree(uv).query(uv, k=min(2, len(uv)))
    local_spacing = np.asarray(neighbor_distances)[:, -1]
    positive = local_spacing[np.isfinite(local_spacing) & (local_spacing > 1.0e-12)]
    if len(positive) == 0:
        raise ValueError("The UV point cloud has no measurable point spacing")
    typical_spacing = float(np.median(positive))
    # Do not let an isolated point make every triangle touching it admissible.
    spacing_limit = np.minimum(local_spacing, 2.5 * typical_spacing)

    triangle_points = uv[simplices]
    ...
    triangle_scale = np.max(spacing_limit[simplices], axis=1)
    kept = np.max(edge_lengths, axis=1) <= alpha_factor * triangle_scale
    if not np.any(kept):
        raise ValueError(
            "The non-convex UV domain is empty; increase --trajectory-alpha-factor"
        )

    # Only the outer boundary constrains coverage.  A rejected-triangle
    # component that reaches a convex-hull edge represents the exterior or an
    # open concavity and remains rejected.  A component fully surrounded by
    # retained triangles is an internal hole in the point sampling; fill it so
    # the coverage path does not detour around missing interior samples.
    rejected = ~kept
    visited = np.zeros(len(simplices), dtype=bool)
    for seed in np.flatnonzero(rejected):
        if visited[seed]:
            continue
        stack = [int(seed)]
        visited[seed] = True
        component: list[int] = []
        touches_outer_hull = False
        while stack:
            triangle_index = stack.pop()
            component.append(triangle_index)
            for neighbor in triangulation.neighbors[triangle_index]:
                if neighbor < 0:
                    touches_outer_hull = True
                elif rejected[neighbor] and not visited[neighbor]:
                    visited[neighbor] = True
                    stack.append(int(neighbor))
        if not touches_outer_hull:
            kept[np.asarray(component, dtype=np.int64)] = True
    return triangulation, kept, typical_spacing


def _scanline_domain_segments(
    uv: np.ndarray,
    ...
    """Return the union of one scanline with all retained UV triangles."""
    intervals: list[tuple[float, float]] = []
    center = normal * offset
    for simplex in simplices[kept]:
        intersection = line_convex_polygon_intersection(center, travel, uv[simplex])
        if intersection is None:
            continue
        values = sorted((float(intersection[0] @ travel), float(intersection[1] @ travel)))
    ...
    return [
        (travel * start + normal * offset, travel * end + normal * offset)
        for start, end in merged
    ]


def _uv_domain_graph(
    uv: np.ndarray, simplices: np.ndarray, kept: np.ndarray,
):
    triangles = simplices[kept]
    edges = np.vstack(
        [triangles[:, [0, 1]], triangles[:, [1, 2]], triangles[:, [2, 0]]]
    )
    edges.sort(axis=1)
    edges = np.unique(edges, axis=0)
    weights = np.linalg.norm(uv[edges[:, 1]] - uv[edges[:, 0]], axis=1)
    rows = np.concatenate([edges[:, 0], edges[:, 1]])
    columns = np.concatenate([edges[:, 1], edges[:, 0]])
    data = np.concatenate([weights, weights])
    return coo_matrix((data, (rows, columns)), shape=(len(uv), len(uv))).tocsr()


def _containing_retained_triangle(
    point: np.ndarray, uv: np.ndarray, simplices: np.ndarray, kept: np.ndarray,
) -> int:
    indices = np.flatnonzero(kept)
    triangles = uv[simplices[indices]]
    a = triangles[:, 0]
    ab = triangles[:, 1] - a
    ac = triangles[:, 2] - a
    ap = point - a
    denominator = ab[:, 0] * ac[:, 1] - ab[:, 1] * ac[:, 0]
    beta = (ap[:, 0] * ac[:, 1] - ap[:, 1] * ac[:, 0]) / denominator
    gamma = (ab[:, 0] * ap[:, 1] - ab[:, 1] * ap[:, 0]) / denominator
    barycentric = np.column_stack([1.0 - beta - gamma, beta, gamma])
    margins = np.min(barycentric, axis=1)
    valid = np.flatnonzero(margins >= -1.0e-8)
    if len(valid):
        return int(indices[valid[np.argmax(margins[valid])]])
    # Roundoff at a scanline endpoint can put it a few ulps outside.  The
    # closest retained triangle is the safe routing anchor in that case.
    centroids = triangles.mean(axis=1)
    return int(indices[np.argmin(np.linalg.norm(centroids - point, axis=1))])


def _segment_stays_in_uv_domain(
    start: np.ndarray,
    end: np.ndarray,
    ...
    count = max(3, int(np.ceil(length / max(sample_step, 1.0e-12))) + 1)
    # Endpoints lie on the domain boundary and can be assigned to the removed
    # triangle on the other side, so test only the connector interior.
    fractions = np.linspace(0.0, 1.0, count)[1:-1]
    if not len(fractions):
        return True
    samples = start + fractions[:, None] * (end - start)
    simplex_ids = triangulation.find_simplex(samples, tol=1.0e-9)
    return bool(np.all((simplex_ids >= 0) & kept[np.maximum(simplex_ids, 0)]))


def _route_connector_inside_uv_domain(
    start: np.ndarray,
    end: np.ndarray,
    uv: np.ndarray,
    triangulation: Delaunay,
    ...
    """Use a direct connector when safe, otherwise route over domain edges."""
    if _segment_stays_in_uv_domain(start, end, triangulation, kept, sample_step):
        return np.vstack([start, end])

    start_triangle = _containing_retained_triangle(
        start, uv, triangulation.simplices, kept
    )
    end_triangle = _containing_retained_triangle(end, uv, triangulation.simplices, kept)
    start_vertices = triangulation.simplices[start_triangle]
    end_vertices = triangulation.simplices[end_triangle]
    distances, predecessors = dijkstra(
        graph, directed=False, indices=start_vertices, return_predecessors=True
    )
    total = (
        np.linalg.norm(uv[start_vertices] - start, axis=1)[:, None]
        + distances[:, end_vertices]
        + np.linalg.norm(uv[end_vertices] - end, axis=1)[None, :]
    )
    source_row, destination_column = np.unravel_index(np.argmin(total), total.shape)
    if not np.isfinite(total[source_row, destination_column]):
        raise ValueError("The non-convex UV domain contains disconnected components")
    source = int(start_vertices[source_row])
    current = int(end_vertices[destination_column])
    vertex_path = [current]
    while current != source:
        current = int(predecessors[source_row, current])
        if current < 0:
            raise ValueError("Cannot route a trajectory connector inside the UV domain")
        vertex_path.append(current)
    vertex_path.reverse()
    connector = np.vstack([start, uv[vertex_path], end])
    keep_points = np.ones(len(connector), dtype=bool)
    keep_points[1:] = np.linalg.norm(np.diff(connector, axis=0), axis=1) > 1.0e-12
    return connector[keep_points]
...
def generate_coverage_trajectory(
    ...
    """Generate an in-domain boustrophedon path over a non-convex UV cloud."""
    if len(uv) < 3:
    ...
    triangulation, kept, point_spacing = build_nonconvex_uv_domain(uv, alpha_factor)
    domain_vertices = np.unique(triangulation.simplices[kept])
    projections = uv[domain_vertices] @ normal
    ...
    line_count = max(2, int(np.ceil(span / spacing)) + 1)
    offsets = np.linspace(projections.min(), projections.max(), line_count)
    actual_spacing = span / (line_count - 1)
    segments: list[tuple[np.ndarray, np.ndarray]] = []
    tolerance = max(point_spacing * 1.0e-7, 1.0e-12)
    for line_index, offset in enumerate(offsets):
        line_segments = _scanline_domain_segments(
            uv, triangulation.simplices, kept, travel, normal,
    ...
    graph = _uv_domain_graph(uv, triangulation.simplices, kept)
    trajectory: list[np.ndarray] = [segments[0][0], segments[0][1]]
    for start, end in segments[1:]:
        connector = _route_connector_inside_uv_domain(
            trajectory[-1], start, uv, triangulation, kept, graph,
            sample_step=0.5 * point_spacing,
        )
        trajectory.extend(connector[1:])
        if np.linalg.norm(trajectory[-1] - end) > 1.0e-12:
            trajectory.append(end)
    return np.asarray(trajectory), float(actual_spacing)
...
def inverse_map_uv_to_xyz(
    ...
    """Map UV to 3-D without extrapolating points inside the UV triangulation.

    Queries inside the sampled UV domain use barycentric interpolation on a
    Delaunay triangle.  Weighted local affine regression is retained only as a
    defensive fallback for numerical queries just outside the convex hull.
    """
    ...
    if distances.shape[0] != len(trajectory_uv):
        ...
        indices = indices.T
    triangulation = Delaunay(sample_uv)
    simplex_ids = triangulation.find_simplex(trajectory_uv, tol=1.0e-9)
    result = np.empty((len(trajectory_uv), 3), dtype=float)
    for row, query in enumerate(trajectory_uv):
        simplex_id = int(simplex_ids[row])
        if simplex_id >= 0:
            transform = triangulation.transform[simplex_id]
            first = transform[:2] @ (query - transform[2])
            barycentric = np.append(first, 1.0 - first.sum())
            result[row] = barycentric @ sample_xyz[triangulation.simplices[simplex_id]]
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--trajectory-alpha-factor", type=float, default=3.0,
        help=(
            "Non-convex UV boundary scale relative to local point spacing; "
            "smaller follows concavities more tightly (default: 3.0)"
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    trajectory_uv, actual_spacing = generate_coverage_trajectory(
        ...
        alpha_factor=args.trajectory_alpha_factor,
    )
    travel_unit = travel_direction / max(
        np.linalg.norm(travel_direction), np.finfo(float).eps
    )
    sweep_normal = np.array([-travel_unit[1], travel_unit[0]])
    sweep_span = float(np.ptp(uv @ sweep_normal))
    coverage_line_count = int(round(sweep_span / actual_spacing)) + 1
    ...
    print("Inverse mapping uses in-domain UV triangle interpolation.")
    ...
    print(f"Non-convex boundary alpha factor: {args.trajectory_alpha_factor:g}")
