...
def _snap_intersection_curve_to_mesh_edges(
    ...
    for (a_raw, b_raw), length in zip(edges, edge_lengths):
        ...
        if b not in boundary_set or b == end:
            rows.append(a); columns.append(b); weights.append(guided_weight)
        if a not in boundary_set or a == end:
            rows.append(b); columns.append(a); weights.append(guided_weight)
...
def _split_mesh_by_plane_intersection(
    mesh: trimesh.Trimesh,
    plane,
    reference_curve: np.ndarray,
) -> tuple[trimesh.Trimesh, np.ndarray, np.ndarray]:
    """Split triangles at the exact plane intersection and build a seam edge path.

    Unlike the historical vertex-graph implementation, this routine never
    replaces an interior plane/edge intersection by its nearest mesh vertex.
    New vertices retain their exact edge parameter, and only triangles crossed
    by the selected intersection component are retriangulated.
    """
    from scipy.spatial import cKDTree
    ...
    distances = vertices @ np.asarray(plane.normal, dtype=float) + float(plane.d)

    edge_nodes: dict[tuple[int, int, int], int] = {}
    node_points: list[np.ndarray] = []
    node_keys: list[tuple[int, int, int]] = []
    face_segments: dict[int, tuple[int, int]] = {}

    def node_for_vertex(vertex: int) -> int:
        key = (0, int(vertex), 0)
        if key not in edge_nodes:
            edge_nodes[key] = -int(vertex) - 1
        return edge_nodes[key]

    def node_for_edge(a: int, b: int, t: float, point: np.ndarray) -> int:
        if a > b:
            a, b, t = b, a, 1.0 - t
        key = (1, (int(a) << 32) ^ int(b), int(round(float(t) * 1.0e12)))
        if key not in edge_nodes:
            edge_nodes[key] = len(node_points)
            node_points.append(np.asarray(point, dtype=float))
            node_keys.append(key)
        return edge_nodes[key]

    def triangle_intersections(face: np.ndarray) -> list[tuple[int, np.ndarray]]:
        result: list[tuple[int, np.ndarray]] = []
        for local in range(3):
            next_local = (local + 1) % 3
            a = int(face[local])
            b = int(face[next_local])
            da = float(distances[a])
            db = float(distances[b])
            pa = vertices[a]
            pb = vertices[b]
            if abs(da) <= eps:
                result.append((node_for_vertex(a), pa.copy()))
            if da * db < -eps * eps:
                t = da / (da - db)
                result.append((node_for_edge(a, b, t, pa + t * (pb - pa)), pa + t * (pb - pa)))
        unique: dict[int, np.ndarray] = {}
        for node, point in result:
            unique[node] = point
        return list(unique.items())

    all_segments: list[tuple[int, int, int]] = []
    for face_index, face in enumerate(faces):
        intersections = triangle_intersections(face)
        if len(intersections) < 2:
            continue
        # A coplanar edge can produce more than two records; retain the
        # farthest pair to form a stable local intersection segment.
        best = max(
            ((a, b) for i, (a, _) in enumerate(intersections)
             for b, _ in intersections[i + 1:]),
            key=lambda pair: float(np.linalg.norm(
                (vertices[-pair[0] - 1] if pair[0] < 0 else node_points[pair[0]])
                - (vertices[-pair[1] - 1] if pair[1] < 0 else node_points[pair[1]])
            )),
        )
        face_segments[face_index] = best
        all_segments.append((face_index, best[0], best[1]))
    if not all_segments:
        raise ValueError("The selected plane produced no triangle-interior intersection segments.")

    def point_of(node: int) -> np.ndarray:
        return vertices[-node - 1] if node < 0 else node_points[node]

    # Select the connected segment component represented by the reference
    # curve, rather than accidentally splitting a different plane component.
    adjacency: dict[int, list[int]] = {}
    for segment_index, (_, a, b) in enumerate(all_segments):
        adjacency.setdefault(a, []).append(segment_index)
        adjacency.setdefault(b, []).append(segment_index)
    components: list[set[int]] = []
    remaining = set(range(len(all_segments)))
    while remaining:
        seed = remaining.pop()
        component = {seed}
        stack = [seed]
        while stack:
            current = stack.pop()
            _, a, b = all_segments[current]
            neighbours = adjacency[a] + adjacency[b]
    ...
    reference_tree = cKDTree(_dense_polyline_samples(reference, max(0.5 * scale / 1000.0, EPS)))
    def component_score(component: set[int]) -> float:
        midpoints = np.vstack([
            0.5 * (point_of(all_segments[i][1]) + point_of(all_segments[i][2]))
            for i in component
        ])
        return float(np.mean(reference_tree.query(midpoints)[0]))
    selected_component = min(components, key=component_score)
    selected_segments = [all_segments[i] for i in selected_component]
    selected_face_segments = {face: (a, b) for face, a, b in selected_segments}

    selected_nodes = sorted({node for _, a, b in selected_segments for node in (a, b)})
    node_to_vertex: dict[int, int] = {node: -node - 1 for node in selected_nodes if node < 0}
    refined_vertices = vertices.tolist()
    for node in selected_nodes:
        if node >= 0:
            node_to_vertex[node] = len(refined_vertices)
            refined_vertices.append(point_of(node).tolist())

    def crossing_node(a: int, b: int) -> int | None:
        da, db = float(distances[a]), float(distances[b])
        if abs(da) <= eps:
            return node_for_vertex(a)
        if abs(db) <= eps:
            return node_for_vertex(b)
        if da * db < -eps * eps:
            t = da / (da - db)
            key_a, key_b = (a, b) if a < b else (b, a)
            key = (1, (int(key_a) << 32) ^ int(key_b), int(round(float(t if a < b else 1.0 - t) * 1.0e12)))
            return edge_nodes.get(key)
        return None

    def clip_polygon(face: np.ndarray, positive: bool) -> list[int]:
        polygon = [int(v) for v in face]
        result: list[int] = []
        for i, current in enumerate(polygon):
            nxt = polygon[(i + 1) % len(polygon)]
            dc = float(distances[current])
            dn = float(distances[nxt])
            inside_c = dc >= -eps if positive else dc <= eps
            inside_n = dn >= -eps if positive else dn <= eps
            if inside_c:
                result.append(current)
            if inside_c != inside_n:
                node = crossing_node(current, nxt)
                if node is not None:
                    result.append(node_to_vertex[node])
    ...
    for face_index, face in enumerate(faces):
        if face_index not in selected_face_segments:
            new_faces.append(face.tolist())
            continue
        original_normal = np.cross(vertices[face[1]] - vertices[face[0]], vertices[face[2]] - vertices[face[0]])
        for positive in (True, False):
            polygon = clip_polygon(face, positive)
            for i in range(1, len(polygon) - 1):
                triangle = [polygon[0], polygon[i], polygon[i + 1]]
                points = np.asarray(refined_vertices, dtype=float)[triangle]
    ...
    refined_mesh = trimesh.Trimesh(
        vertices=np.asarray(refined_vertices, dtype=float),
        faces=np.asarray(new_faces, dtype=np.int64), process=False,
    )

    # Trace the selected segment component from its boundary endpoint.  For a
    # boundary-to-boundary plane cut this is a single chain, not a vertex snap.
    seam_adjacency: dict[int, list[int]] = {}
    for _, a, b in selected_segments:
        seam_adjacency.setdefault(a, []).append(b)
        seam_adjacency.setdefault(b, []).append(a)
    endpoints = [node for node, neighbours in seam_adjacency.items() if len(neighbours) == 1]
    if len(endpoints) < 2:
        raise ValueError("The selected plane intersection is not an open boundary-to-boundary chain.")
    start = min(endpoints, key=lambda node: float(np.linalg.norm(point_of(node) - reference[0])))
    path_nodes = [start]
    previous = None
    current = start
    while True:
        choices = [node for node in seam_adjacency[current] if node != previous]
        if not choices:
            break
        next_node = choices[0]
        path_nodes.append(next_node)
        previous, current = current, next_node
    path_vertices = np.asarray([node_to_vertex[node] for node in path_nodes], dtype=np.int64)
    return refined_mesh, path_vertices, np.asarray([point_of(node) for node in path_nodes], dtype=float)
...
def _boundary_uv_indices(uv_faces: np.ndarray) -> set[int]:
    counts: dict[tuple[int, int], int] = {}
    for triangle in np.asarray(uv_faces, dtype=np.int64):
        for a_raw, b_raw in zip(triangle, np.roll(triangle, -1)):
            a, b = int(a_raw), int(b_raw)
            key = (min(a, b), max(a, b))
...
def _planning_frame(seam_segments: Sequence[SeamUVSegment]) -> PlanningFrame:
    ...
    for segment in seam_segments:
        for ratio in (0.0, 0.5, 1.0):
            ...
            samples.append((float(np.linalg.norm(uv_b - uv_a)), segment.physical_index, uv_a, uv_b))
...
def _balanced_center_positions(width: float, target_spacing: float) -> tuple[np.ndarray, float]:
    estimated = max(1, int(round(width / target_spacing)))
    # The center snake must contain an odd number of tracks so that it enters
    # from the lower side and exits from the upper side. Since track count is
    # transition_count + 1, choose the closest positive even transition count.
    candidates = sorted({
        max(2, estimated - 1 if estimated % 2 else estimated),
        max(2, estimated + 1 if estimated % 2 else estimated),
    })
    transition_count = min(
        candidates,
        key=lambda count: abs(width / count - target_spacing),
    )
    spacing = width / transition_count
    return np.linspace(0.0, width, transition_count + 1), spacing
...
def _balanced_outer_positions(
    ...
    usable = max(0.0, float(extent) - 0.5 * target_spacing)
    if usable <= EPS or count <= 0:
        return np.empty(0, dtype=float), float("nan")
    actual = usable / count
    offsets = actual * np.arange(1, count + 1, dtype=float)
    return waist_x + direction * offsets, actual


def _shared_outer_count(left_extent: float, right_extent: float, spacing: float) -> int:
    left_usable = max(0.0, left_extent - 0.5 * spacing)
    right_usable = max(0.0, right_extent - 0.5 * spacing)
    if min(left_usable, right_usable) <= EPS:
        return 0
    return max(1, int(round(0.5 * (left_usable + right_usable) / spacing)))
...
def _overtravel_segment(
    ...
    xyz = np.vstack([
        previous.xyz[-1],
        previous.xyz[-1] + overtravel * exit_direction,
        following.xyz[0] - overtravel * entry_direction,
        following.xyz[0],
    ])
...
def _make_surface_scan(
    ...
    return TrajectorySegment("SURFACE_SCAN", uv, mapper.map(uv), True, note)
...
def _make_scan_from_seam_destination(
    ...
    # Find ALL intervals at this x, then pick the one containing dest_y (most robust)
    all_intervals = _line_domain_intervals(local_uv, uv_faces, x_val, tolerance)
    if not all_intervals:
        return None  # No mesh at this x position

    # Priority 1: interval that contains dest_y
    containing = [iv for iv in all_intervals if iv[0] - tolerance <= dest_y <= iv[1] + tolerance]
    if containing:
        interval = max(containing, key=lambda iv: iv[1] - iv[0])  # Pick longest if multiple
    else:
        # Priority 2: use region-based heuristic
        interval = _region_interval(local_uv, uv_faces, x_val, region, tolerance)
        if interval is None:
            # Priority 3: fallback to largest interval
            interval = max(all_intervals, key=lambda iv: iv[1] - iv[0])
...
def generate_seam_aware_trajectory(
    ...
    lower_count = _shared_outer_count(extents["lower_left"], extents["lower_right"], target_spacing)
    upper_count = _shared_outer_count(extents["upper_left"], extents["upper_right"], target_spacing)
    lower_left_x, lower_left_spacing = _balanced_outer_positions(
        extents["lower_left"], lower_count, 0.0, -1.0, target_spacing
    )
    lower_right_x, lower_right_spacing = _balanced_outer_positions(
        extents["lower_right"], lower_count, frame.waist_width, 1.0, target_spacing
    )
    upper_left_x, upper_left_spacing = _balanced_outer_positions(
        extents["upper_left"], upper_count, 0.0, -1.0, target_spacing
    )
    upper_right_x, upper_right_spacing = _balanced_outer_positions(
        extents["upper_right"], upper_count, frame.waist_width, 1.0, target_spacing
    )
    center_x, center_spacing = _balanced_center_positions(frame.waist_width, target_spacing)
    ...
    center = _available_tracks(center_x, "full", local_uv, data.uv_faces, tolerance)
    paired_lower = max(len(lower_left), len(lower_right))
    paired_upper = max(len(upper_left), len(upper_right))
    if not center:
        raise ValueError("No central scan line intersects the real UV domain.")
    ...
    for index in range(paired_lower):
        ...
        first = _make_surface_scan(
            *first_track, True, sample_step, frame, mapper, note=labels[0]
        )
        append_surface(first, connect_from_previous=(last_surface is not None))
        destination = append_jump(first.uv[-1], from_a=first_is_a)
        second = _make_scan_from_seam_destination(
            destination, "lower", False, sample_step,
            frame, local_uv, data.uv_faces, tolerance, mapper, note=labels[1],
        )
        if second is not None:
            append_surface(second, connect_from_previous=False)

    # Central equal-width region: ordinary bow-shaped vertical snake.
    for index, track in enumerate(center):
    ...
    # At the upper waist, work outwards.  A downward pass reaches one seam
    # copy, then resumes upward from the equivalent point on the other copy.
    for index in range(paired_upper):
        left_available = index < len(upper_left)
        right_available = index < len(upper_right)

        # Alternate starting side for better coverage balance
        prefer_right_first = index % 2 == 0

        if prefer_right_first and right_available:
            first_track = upper_right[index]
            first_is_a = False
            labels = ("upper-right downward", "upper-left upward (seam-driven)")
        elif not prefer_right_first and left_available:
            ...
            first_track = upper_right[index]
            first_is_a = False
            labels = ("upper-right downward (unpaired)", "upper-left upward (seam-driven)")
        elif left_available:
            first_track = upper_left[index]
            first_is_a = True
            labels = ("upper-left downward (unpaired)", "upper-right upward (seam-driven)")
        else:
            continue  # Should never happen given paired_upper = max(...)

        first = _make_surface_scan(
            *first_track, False, sample_step, frame, mapper, note=labels[0]
        )
        append_surface(
            first,
            connect_from_previous=(last_surface is not None),
    ...
    metadata: dict[str, object] = {
        "requested_spacing": target_spacing,
...
def _play_iteration_animation_matplotlib_legacy(mesh: trimesh.Trimesh, result_obj: Path) -> None:
...
def _show_result_matplotlib_legacy(
    ...
    if trajectory:
        ...
        for segment in trajectory:
            if len(segment.xyz) < 2:
                continue
            label = segment.kind if segment.kind not in shown_kinds else None
            shown_kinds.add(segment.kind)
            axes_3d.plot(
                segment.xyz[:, 0], segment.xyz[:, 1], segment.xyz[:, 2],
                color=colors.get(segment.kind, "black"),
                linewidth=1.7 if segment.kind == "SURFACE_SCAN" else 1.1,
                linestyle="--" if segment.kind == "OVERTRAVEL" else "-",
                label=label,
            )
        surface_segments = [item for item in trajectory if item.kind == "SURFACE_SCAN" and len(item.xyz) >= 2]
        if surface_segments:
            start_xyz = surface_segments[0].xyz[0]
            end_xyz = surface_segments[-1].xyz[-1]
            surface_xyz = np.vstack([item.xyz for item in surface_segments])
            trajectory_scale = max(float(np.ptp(surface_xyz, axis=0).max()), 1.0)
            axes_3d.scatter(*start_xyz, color="limegreen", s=55, marker="o", label="start", zorder=8)
            axes_3d.scatter(*end_xyz, color="magenta", s=65, marker="X", label="end", zorder=8)
            arrow_stride = max(1, len(surface_segments) // 12)
            for segment in surface_segments[::arrow_stride]:
                middle = max(0, min(len(segment.xyz) - 2, len(segment.xyz) // 2))
                origin = segment.xyz[middle]
                direction = segment.xyz[middle + 1] - origin
                axes_3d.quiver(
                    *origin, *direction, color="#ff7f0e", normalize=True,
                    length=trajectory_scale * 0.035, arrow_length_ratio=0.35,
                )
        axes_3d.legend(loc="best")
    ...
    if trajectory:
        ...
        jump_label_used = False
        for segment in trajectory:
            if segment.kind == "SURFACE_SCAN" and len(segment.uv) >= 2:
                axes_uv.plot(
                    segment.uv[:, 0], segment.uv[:, 1], color="#d62728", linewidth=1.35,
                    label=None if surface_label_used else "surface scan",
                )
                surface_label_used = True
            elif segment.kind == "SEAM_JUMP" and len(segment.uv) == 2:
                # The two markers are equivalent; deliberately do not draw a
                # false straight line across the developed UV chart.
                axes_uv.scatter(
                    segment.uv[:, 0], segment.uv[:, 1], s=16, color="#1f77b4",
                    label=None if jump_label_used else "SEAM_JUMP pair", zorder=5,
                )
                jump_label_used = True
        surface_segments = [item for item in trajectory if item.kind == "SURFACE_SCAN" and len(item.uv) >= 2]
        if surface_segments:
            start_uv = surface_segments[0].uv[0]
            end_uv = surface_segments[-1].uv[-1]
            axes_uv.scatter(*start_uv, color="limegreen", s=55, marker="o", label="start", zorder=7)
            axes_uv.scatter(*end_uv, color="magenta", s=65, marker="X", label="end", zorder=7)
            arrow_stride = max(1, len(surface_segments) // 16)
            for segment in surface_segments[::arrow_stride]:
                middle = max(0, min(len(segment.uv) - 2, len(segment.uv) // 2))
                origin = segment.uv[middle]
                direction = segment.uv[middle + 1] - origin
                axes_uv.annotate(
                    "", xy=origin + direction, xytext=origin,
                    arrowprops={"arrowstyle": "->", "color": "#ff7f0e", "lw": 1.2},
                    zorder=6,
                )
        if planning_frame is not None:
            ...
            axes_uv.plot(reference[:, 0], reference[:, 1], color="#17becf", linewidth=2.0,
                         label="reference line 1")
        axes_uv.legend(loc="best")
    axes_uv.set_aspect("equal", adjustable="box")
    axes_uv.set_xlabel("u")
    axes_uv.set_ylabel("v")
    axes_uv.set_title("2D OptCuts UV trajectory")
    axes_3d.set_title("3D seam-aware spray trajectory")
    figure.tight_layout()
    plt.show()


# The legacy Matplotlib viewers are retained above for reference only. Runtime
# display uses the VTK implementations below.
def _require_vtk():
    try:
        import vtk
        ...
        raise RuntimeError(
            "VTK is required for display. Install it with: pip install vtk"
        ) from exc
    return vtk, numpy_to_vtk


def _vtk_triangle_polydata(vertices: np.ndarray, faces: np.ndarray):
    vtk, numpy_to_vtk = _require_vtk()
    ...
    for face in np.asarray(faces, dtype=np.int64):
        ...
        for index, vertex_id in enumerate(face):
            triangle.GetPointIds().SetId(index, int(vertex_id))
...
def _vtk_polyline_actor(segments, color, width=2.0, dashed=False):
    vtk, numpy_to_vtk = _require_vtk()
    valid = [np.asarray(item, dtype=np.float64) for item in segments if len(item) >= 2]
    points_array = np.vstack(valid) if valid else np.empty((0, 3), dtype=np.float64)
    points = vtk.vtkPoints()
    points.SetData(numpy_to_vtk(np.ascontiguousarray(points_array), deep=True))
    lines = vtk.vtkCellArray()
    offset = 0
    for segment in valid:
        line = vtk.vtkPolyLine()
        line.GetPointIds().SetNumberOfIds(len(segment))
        for local_index in range(len(segment)):
            line.GetPointIds().SetId(local_index, offset + local_index)
...
def _vtk_marker_actor(point: np.ndarray, radius: float, color):
    vtk, _ = _require_vtk()
    source = vtk.vtkSphereSource()
    source.SetCenter(*map(float, point))
...
def _vtk_points_actor(points_array: np.ndarray, color, size=7.0):
    vtk, numpy_to_vtk = _require_vtk()
    values = np.ascontiguousarray(points_array, dtype=np.float64)
    points = vtk.vtkPoints()
    points.SetData(numpy_to_vtk(values, deep=True))
...
def _vtk_title(renderer, text: str) -> None:
    vtk, _ = _require_vtk()
    title = vtk.vtkTextActor()
    title.SetInput(text)
    title.SetPosition(18, 18)
    title.GetTextProperty().SetFontSize(20)
    title.GetTextProperty().SetColor(0.92, 0.92, 0.92)
    renderer.AddActor2D(title)


def _show_result_vtk_legacy(
    mesh: trimesh.Trimesh,
    uv: np.ndarray,
    uv_faces: np.ndarray,
    cut_edges: np.ndarray | None = None,
    ...
    """Show the 3D mesh and UV chart using VTK's OpenGL renderer."""
    vtk, _ = _require_vtk()
    vertices = np.asarray(mesh.vertices, dtype=np.float64)
    faces = np.asarray(mesh.faces, dtype=np.int64)
    renderer_3d = vtk.vtkRenderer()
    renderer_uv = vtk.vtkRenderer()
    renderer_3d.SetViewport(0.0, 0.0, 0.5, 1.0)
    renderer_uv.SetViewport(0.5, 0.0, 1.0, 1.0)
    renderer_3d.SetBackground(0.07, 0.08, 0.10)
    renderer_uv.SetBackground(0.96, 0.96, 0.96)

    mesh_mapper = vtk.vtkPolyDataMapper()
    mesh_mapper.SetInputData(_vtk_triangle_polydata(vertices, faces))
    mesh_actor = vtk.vtkActor()
    mesh_actor.SetMapper(mesh_mapper)
    mesh_actor.GetProperty().SetColor(0.68, 0.78, 0.88)
    mesh_actor.GetProperty().SetOpacity(0.70)
    renderer_3d.AddActor(mesh_actor)
    if cut_edges is not None and len(cut_edges):
        renderer_3d.AddActor(_vtk_polyline_actor(cut_edges, (0.90, 0.05, 0.08), 3.0))

    colors = {
        "SURFACE_SCAN": (0.84, 0.16, 0.16),
        "SEAM_JUMP": (0.12, 0.47, 0.71),
        "OVERTRAVEL": (0.17, 0.63, 0.17),
    }
    trajectory = list(trajectory or [])
    for kind in colors:
        xyz_segments = [item.xyz for item in trajectory if item.kind == kind]
        if xyz_segments:
            renderer_3d.AddActor(_vtk_polyline_actor(
                xyz_segments, colors[kind], 3.0 if kind == "SURFACE_SCAN" else 2.0,
                dashed=kind == "OVERTRAVEL",
            ))

    surface_segments = [item for item in trajectory if item.kind == "SURFACE_SCAN" and len(item.xyz) >= 2]
    if surface_segments:
        scale = max(float(np.ptp(np.vstack([item.xyz for item in surface_segments]), axis=0).max()), 1.0)
        renderer_3d.AddActor(_vtk_marker_actor(surface_segments[0].xyz[0], scale * 0.012, (0.1, 0.9, 0.2)))
        renderer_3d.AddActor(_vtk_marker_actor(surface_segments[-1].xyz[-1], scale * 0.014, (0.95, 0.1, 0.8)))

    uv3 = np.column_stack([np.asarray(uv, dtype=np.float64), np.zeros(len(uv))])
    uv_mapper = vtk.vtkPolyDataMapper()
    uv_mapper.SetInputData(_vtk_triangle_polydata(uv3, uv_faces))
    uv_actor = vtk.vtkActor()
    uv_actor.SetMapper(uv_mapper)
    uv_actor.GetProperty().SetRepresentationToWireframe()
    uv_actor.GetProperty().SetColor(0.15, 0.15, 0.15)
    renderer_uv.AddActor(uv_actor)
    uv_elevation = max(float(np.ptp(uv, axis=0).max()), 1.0) * 1.0e-4
    uv_segments = [
        np.column_stack([item.uv, np.full(len(item.uv), uv_elevation)])
        for item in trajectory if item.kind == "SURFACE_SCAN" and len(item.uv) >= 2
    ]
    if uv_segments:
        renderer_uv.AddActor(_vtk_polyline_actor(uv_segments, colors["SURFACE_SCAN"], 3.0))
    seam_jump_points = [
        np.column_stack([item.uv, np.full(len(item.uv), 1.5 * uv_elevation)])
        for item in trajectory if item.kind == "SEAM_JUMP" and len(item.uv) == 2
    ]
    if seam_jump_points:
        renderer_uv.AddActor(_vtk_points_actor(
            np.vstack(seam_jump_points), colors["SEAM_JUMP"], 8.0
        ))
    uv_surface_segments = [
        item for item in trajectory if item.kind == "SURFACE_SCAN" and len(item.uv) >= 2
    ]
    if uv_surface_segments:
        uv_scale = max(float(np.ptp(uv, axis=0).max()), 1.0)
        for point, color in (
            (uv_surface_segments[0].uv[0], (0.1, 0.9, 0.2)),
            (uv_surface_segments[-1].uv[-1], (0.95, 0.1, 0.8)),
        ):
            renderer_uv.AddActor(_vtk_marker_actor(
                np.array([point[0], point[1], 2.0 * uv_elevation]),
                uv_scale * 0.009,
                color,
            ))
    if planning_frame is not None:
        ...
        reference3 = np.column_stack([reference, np.full(2, 2.0 * uv_elevation)])
        renderer_uv.AddActor(_vtk_polyline_actor([reference3], (0.09, 0.75, 0.81), 3.0))

    _vtk_title(renderer_3d, "3D seam-aware spray trajectory")
    _vtk_title(renderer_uv, "2D OptCuts UV trajectory")
    renderer_3d.ResetCamera()
    renderer_uv.ResetCamera()
    renderer_uv.GetActiveCamera().ParallelProjectionOn()
    window = vtk.vtkRenderWindow()
    window.SetWindowName("OptCuts trajectory - VTK/OpenGL")
    window.SetSize(1500, 820)
    window.SetMultiSamples(4)
    window.AddRenderer(renderer_3d)
    window.AddRenderer(renderer_uv)
...
def _play_iteration_animation_vtk_legacy(mesh: trimesh.Trimesh, result_obj: Path) -> None:
    """Play OptCuts mesh frames using one persistent VTK/OpenGL pipeline."""
    del mesh  # Bounds are obtained from the first rendered frame.
    vtk, numpy_to_vtk = _require_vtk()
    ...
    if not frame_paths:
        print(f"OptCuts 3D frames were not found: {frame_directory}")
        return
    vertices, faces, _, _ = _read_iteration_obj(frame_paths[0])
    polydata = _vtk_triangle_polydata(vertices, faces)
    mapper = vtk.vtkPolyDataMapper()
    mapper.SetInputData(polydata)
    actor = vtk.vtkActor()
    actor.SetMapper(mapper)
    actor.GetProperty().SetColor(0.72, 0.82, 0.92)
    ...
    renderer.AddActor2D(title)
    window = vtk.vtkRenderWindow()
    window.SetWindowName("OptCuts iteration - VTK/OpenGL")
    window.SetSize(1100, 820)
    window.AddRenderer(renderer)
    interactor = vtk.vtkRenderWindowInteractor()
    interactor.SetRenderWindow(window)
    interactor.SetInteractorStyle(vtk.vtkInteractorStyleTrackballCamera())
    state = {"index": 0, "faces": np.asarray(faces)}

    def set_frame(frame_index: int) -> None:
        current_vertices, current_faces, _, _ = _read_iteration_obj(frame_paths[frame_index])
        vtk_points = vtk.vtkPoints()
        vtk_points.SetData(numpy_to_vtk(
            np.ascontiguousarray(current_vertices, dtype=np.float64), deep=True
        ))
        polydata.SetPoints(vtk_points)
        if not np.array_equal(current_faces, state["faces"]):
            replacement = _vtk_triangle_polydata(current_vertices, current_faces)
            polydata.SetPolys(replacement.GetPolys())
            state["faces"] = np.asarray(current_faces).copy()
        polydata.Modified()
        title.SetInput(f"OptCuts iteration {frame_index + 1}/{len(frame_paths)}")

    def on_timer(_caller, _event) -> None:
        state["index"] = (state["index"] + 1) % len(frame_paths)
...
def _show_result(
    ...
    show_result(
        ...
        trajectory=trajectory, planning_frame=planning_frame,
    )
...
def main() -> None:
    ...
    trajectory, planning_frame, metadata = generate_seam_aware_trajectory(
        data, precut_seam, target_spacing=args.spacing, sample_step=sample_step,
    )
