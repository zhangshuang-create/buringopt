...
def vtk_circle(center: np.ndarray, normal: np.ndarray, radius: float,
               segments: int = 128) -> vtk.vtkPolyData:
    """Create a 3D circle in the plane with ``normal``."""
    center = np.asarray(center, dtype=float)
    u_axis, v_axis = _plane_basis(np.asarray(normal, dtype=float))
    angles = np.linspace(0.0, 2.0 * math.pi, max(16, int(segments)) + 1)
    points = center[None, :] + radius * (
        np.cos(angles)[:, None] * u_axis[None, :] +
        np.sin(angles)[:, None] * v_axis[None, :]
    )
    return vtk_polyline(points)


def vtk_sphere(center: np.ndarray, radius: float) -> vtk.vtkActor:
    """Create a small sphere actor for a 3D debug marker."""
    source = vtk.vtkSphereSource()
    source.SetCenter(*map(float, np.asarray(center, dtype=float)))
    source.SetRadius(float(radius))
    source.SetThetaResolution(24)
    source.SetPhiResolution(16)
    source.Update()
    return vtk_actor(source.GetOutput(), (0.1, 0.35, 0.95), opacity=0.95)
...
class InteractiveVisualizer:
    ...
    def update_trajectory(self, trajectory: Sequence[TrajectorySegment], planning_frame: PlanningFrame,
        ...
        # === 3D upper-boundary center / fixed-center maximum circle debug view ===
        # The radius is the first-contact distance from top.center to the
        # actual upper-boundary polyline.  This is intentionally recomputed
        # from the current mesh so the marker reflects the cut input geometry.
        try:
            boundary_loops = extract_boundary_loops(self.mesh)
            top_boundary, _ = classify_top_bottom_boundaries(boundary_loops)
            top_center, top_axes, _ = compute_pca(top_boundary.points)
            green_axis = _unit(top_axes[1], "upper-boundary PCA green axis")
            circle_radius = float("inf")
            for edge_start, edge_stop in zip(
                    top_boundary.points,
                    np.roll(top_boundary.points, -1, axis=0)):
                edge = edge_stop - edge_start
                denom = float(np.dot(edge, edge))
                ratio = 0.0 if denom <= EPS else float(np.clip(
                    np.dot(top_center - edge_start, edge) / denom, 0.0, 1.0))
                candidate = edge_start + ratio * edge
                circle_radius = min(circle_radius, float(np.linalg.norm(candidate - top_center)))
            model_span = max(float(np.linalg.norm(np.ptp(self.vertices_3d, axis=0))), 1.0)
            marker_radius = max(0.012 * model_span, 0.25 * self.sample_step)
            center_actor = vtk_sphere(top_center, marker_radius)
            self.left_overlay_renderer.AddActor(center_actor)
            self.trajectory_actors_3d.append(center_actor)
            if circle_radius > EPS:
                circle_actor = vtk_actor(
                    vtk_circle(top_center, top_boundary.normal, circle_radius),
                    (1.0, 0.75, 0.05), width=3.0,
                )
                self.left_overlay_renderer.AddActor(circle_actor)
                self.trajectory_actors_3d.append(circle_actor)
        except (ValueError, np.linalg.LinAlgError):
            # Debug geometry must never prevent trajectory visualization.
            pass
