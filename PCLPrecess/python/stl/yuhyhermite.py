...
class TrajectorySegment:
    ...
    phases: Optional[np.ndarray] = None  # 【新增】：与 xyz 等长的 1D int 数组，0: 前半段/Master，1: 后半段/Slave
...
def fit_superellipse_v_trajectories(
    ...
    1. 搜索范围：指数 n 允许在 [0.4, 2.2] 之间自动优化（涵盖星形线、摆线三角形、勒洛三角形到标准椭圆）；
    2. 位移约束：顶点 P2 沿角平分线向底边移动的距离严格限制在 [0, 2*d]；
    3. 端点约束：两端 P1, P3 严格固定不发生任何位移。
    """
    from scipy.optimize import minimize

    d = float(target_spacing)
    d_max_shift = 2.0 * d  # 顶点最大允许内移 2d

    optimized: List[TrajectorySegment] = []

    for item in extracted_v_data:
        if item["type"] == "PASSTHROUGH":
            optimized.append(item["seg"])
            continue
        ...
        # 1. 建立局部 2D 坐标系 (以 P1-P3 中点为原点)
        M = 0.5 * (P1 + P3)
        X_axis = P3 - P1
        half_width = np.linalg.norm(X_axis) / 2.0
        if half_width < 1e-6:
            optimized.append(seg)
            continue
        X_dir = X_axis / (2.0 * half_width)
        ...
        # 投影原始轨迹点到局部 2D 坐标以进行最优残差拟合
        vecs_raw = xyz_raw - M
        x_raw_2d = vecs_raw @ X_dir
        y_raw_2d = vecs_raw @ Y_dir

        a = half_width

        # 2. 定义超椭圆曲线生成器
        def generate_curve_2d(n_val, h_val, n_pts):
            t = np.linspace(math.pi, 0.0, n_pts)
            cos_t = np.cos(t)
            sin_t = np.sin(t)
            exp_factor = 2.0 / n_val
            x_loc = a * np.sign(cos_t) * (np.abs(cos_t) ** exp_factor)
            y_loc = h_val * (np.abs(sin_t) ** exp_factor)
            return x_loc, y_loc

        # 3. 最佳参数优化目标函数 (求解最优指数 n 和顶点高度 h)
        # 顶点向内微调最大不得超过 2d，且不能低于原高度的 30%
        min_allowed_h = max(raw_height * 0.3, raw_height - d_max_shift)
        max_allowed_h = raw_height

        def fitting_loss(params):
            n_curr, h_curr = params
            x_loc, y_loc = generate_curve_2d(n_curr, h_curr, len(x_raw_2d))
            # 拟合形状残差
            return np.mean((x_loc - x_raw_2d) ** 2 + (y_loc - y_raw_2d) ** 2)

        # 初始猜测：n=1.0 (星形线向圆弧过渡态)，h = raw_height
        init_guess = [1.0, raw_height]
        bounds = [(0.4, 2.2), (min_allowed_h, max_allowed_h)]

        opt_res = minimize(fitting_loss, init_guess, bounds=bounds, method='L-BFGS-B')
        best_n, best_h = opt_res.x
        shift_amount = raw_height - best_h

        # 4. 根据最佳形态重采样生成 3D 轨迹
        arc_length_est = math.pi * math.sqrt(0.5 * (a ** 2 + best_h ** 2))
        step = sample_step if sample_step > 1e-6 else 1.0
        num_samples = max(24, int(np.ceil(arc_length_est / step)))

        x_opt_loc, y_opt_loc = generate_curve_2d(best_n, best_h, num_samples)

        curve_3d = M[None, :] + np.outer(x_opt_loc, X_dir) + np.outer(y_opt_loc, Y_dir)
        ...
        optimized.append(TrajectorySegment(
            ...
            note=seg.note + f" [Optimal Fit n={best_n:.2f}, shift={shift_amount:.1f}mm]",
            region=seg.region,
            phases=phases
        ))
...
def fit_superellipse_v_trajectories(
    ...
    """
    全形态自适应拟合：全面支持 三角内摆线 (n<1)、直角三角 (n=1)、勒洛三角 (1<n<2) 与标准圆/椭圆 (n=2)。

    1. 搜索空间：n 范围严格限制在 (0, 2.0]（数值边界设为 [0.05, 2.0]）；
    2. 几何约束：顶点 P2 沿角平分线向内收缩距离严格限制在 [0, 2*d]；
    3. 端点约束：P1、P3 严格锁定不动。
    """
    from scipy.optimize import minimize
    ...
    for item in extracted_v_data:
        ...
        # 2. 均匀弧长投影插值生成器
        def generate_curve_2d(n_val, h_val, n_pts):
            t = np.linspace(math.pi, 0.0, max(200, n_pts * 4))
            cos_t = np.cos(t)
            sin_t = np.sin(t)
            exp_factor = 2.0 / max(n_val, 0.05)

            x_dense = a * np.sign(cos_t) * (np.abs(cos_t) ** exp_factor)
            y_dense = h_val * (np.abs(sin_t) ** exp_factor)

            # 按累计弧长均匀化点分布
            ds = np.hypot(np.diff(x_dense), np.diff(y_dense))
            s_cum = np.r_[0.0, np.cumsum(ds)]
            s_uniform = np.linspace(0.0, s_cum[-1], n_pts)

            x_loc = np.interp(s_uniform, s_cum, x_dense)
            y_loc = np.interp(s_uniform, s_cum, y_dense)
            return x_loc, y_loc

        # 3. 优化器：全局搜索最优 n (0~2) 与顶点高度 h
        min_allowed_h = max(raw_height * 0.3, raw_height - d_max_shift)
        max_allowed_h = raw_height
        ...
        def fitting_loss(params):
            ...
            err_fit = np.mean((x_loc - x_raw_2d) ** 2 + (y_loc - y_raw_2d) ** 2)

            # 腰部区域 (x 在 [-0.6a, 0.6a] 范围内) 赋予更高权重
            mid_mask = np.abs(x_loc) < 0.6 * a
            if np.any(mid_mask):
                mid_err = np.mean((y_loc[mid_mask] - np.interp(x_loc[mid_mask], x_raw_2d, y_raw_2d)) ** 2)
            else:
                mid_err = 0.0

            return err_fit + 1.5 * mid_err

        # 初始猜测 n=0.65 (三角内摆线区域)，严格限制 n 在 [0.05, 2.0]
        init_guess = [0.65, raw_height - 0.5 * min(d, raw_height * 0.1)]
        bounds = [(0.05, 2.0), (min_allowed_h, max_allowed_h)]
        ...
        arc_length_est = math.pi * math.sqrt(0.5 * (a ** 2 + best_h ** 2))
        step = sample_step if sample_step > 1e-6 else 1.0
        num_samples = max(32, int(np.ceil(arc_length_est / step)))
        ...
        # 判定形态名称 (映射至 0~2 区间)
        if best_n < 0.85:
            shape_type = "三角内摆线 (Deltoid)"
        elif best_n <= 1.15:
            shape_type = "近标准三角 (Linear)"
        elif best_n < 1.95:
            shape_type = "勒洛三角 (Reuleaux)"
        else:
            shape_type = "标准圆/椭圆 (Circle/Ellipse)"
        ...
        optimized.append(TrajectorySegment(
            ...
            xyz=curve_3d,
            spray_on=seg.spray_on,
            note=seg.note + f" [{shape_type} n={best_n:.2f}, shift={shift_amount:.1f}mm]",
            region=seg.region,
            phases=phases
        ))
...
def optimize_trajectory_arcs_postprocess(
    ...
    """用满足运动学约束的内切圆弧替换 LOWER/UPPER 区域的 V 形尖角。"""
    # 保留这些参数以兼容原有调用接口。严格空间圆弧不能逐点投影到一般曲面，
    # 否则投影后的点不再具有相同的圆心和半径。
    del mesh, smooth_iterations, smooth_weight

    if not np.isfinite(speed) or speed <= 0.0:
        raise ValueError("speed must be greater than zero")
    if not np.isfinite(a_max) or a_max <= 0.0:
        raise ValueError("a_max must be greater than zero")
    if not np.isfinite(j_max) or j_max <= 0.0:
        raise ValueError("j_max must be greater than zero")

    radius_from_acceleration = speed ** 2 / a_max
    radius_from_jerk = math.sqrt(speed ** 3 / j_max)
    minimum_radius = max(radius_from_acceleration, radius_from_jerk)

    print(
        "[Motion Constraint] "
        f"v={speed:.3f}, a_max={a_max:.3f}, j_max={j_max:.3f}, "
        f"R_a={radius_from_acceleration:.3f}, "
        f"R_j={radius_from_jerk:.3f}, R_min={minimum_radius:.3f} mm"
    )
    ...
    for seg in trajectory:
        if (
                seg.kind != "SURFACE_SCAN"
                or getattr(seg, "region", "") not in ("LOWER", "UPPER")
        ):
            optimized.append(seg)
            continue
        ...
        projection_lengths = (xyz - P1) @ chord_unit
        chord_projection = P1 + np.outer(projection_lengths, chord_unit)
        idx_tip = int(np.argmax(np.linalg.norm(xyz - chord_projection, axis=1)))
        if idx_tip <= 0 or idx_tip >= len(xyz) - 1:
            optimized.append(seg)
            continue
        ...
        first_arm = P1 - P2
        second_arm = P3 - P2
        first_length = float(np.linalg.norm(first_arm))
        second_length = float(np.linalg.norm(second_arm))
        if first_length < 1e-9 or second_length < 1e-9:
            optimized.append(seg)
            continue

        first_unit = first_arm / first_length
        second_unit = second_arm / second_length
        theta = math.acos(float(np.clip(np.dot(first_unit, second_unit), -1.0, 1.0)))
        ...
        if abs(sin_half) < 1e-9 or abs(tan_half) < 1e-9:
            optimized.append(seg)
            continue

        # 图示几何关系：L1 为顶点到圆心，L2 为顶点到两个切点。
        radius = minimum_radius
        L1 = radius / sin_half
        L2 = radius / tan_half

        available_length = min(first_length, second_length)
        if L2 >= available_length - 1e-6:
            print(
                f"[Fillet skipped] region={seg.region}: required L2={L2:.3f} mm, "
                f"available={available_length:.3f} mm, R_min={radius:.3f} mm"
            )
            optimized.append(seg)
            continue

        first_tangent = P2 + first_unit * L2
        second_tangent = P2 + second_unit * L2

        angle_bisector = first_unit + second_unit
        bisector_length = float(np.linalg.norm(angle_bisector))
        if bisector_length < 1e-9:
            optimized.append(seg)
            continue
        center = P2 + angle_bisector / bisector_length * L1

        start_radius = first_tangent - center
        end_radius = second_tangent - center
        start_radius_length = float(np.linalg.norm(start_radius))
        end_radius_length = float(np.linalg.norm(end_radius))
        radius_tolerance = max(1e-6, radius * 1e-6)
        if (
                abs(start_radius_length - radius) > radius_tolerance
                or abs(end_radius_length - radius) > radius_tolerance
        ):
            print(
                f"[Fillet skipped] invalid geometry: R={radius:.6f}, "
                f"|C-T1|={start_radius_length:.6f}, |C-T2|={end_radius_length:.6f}"
            )
            optimized.append(seg)
            continue

        start_unit = start_radius / start_radius_length
        end_unit = end_radius / end_radius_length
        cross_radius = np.cross(start_unit, end_unit)
        cross_length = float(np.linalg.norm(cross_radius))
        if cross_length < 1e-9:
            optimized.append(seg)
            continue

        rotation_axis = cross_radius / cross_length
        arc_angle = math.atan2(cross_length, float(np.dot(start_unit, end_unit)))
        expected_arc_angle = math.pi - theta
        if not np.isclose(arc_angle, expected_arc_angle, rtol=1e-5, atol=1e-7):
            arc_angle = expected_arc_angle

        effective_step = sample_step if np.isfinite(sample_step) and sample_step > 1e-6 else 1.0
        arc_segment_count = max(8, int(math.ceil(radius * arc_angle / effective_step)))
        arc_points = np.empty((arc_segment_count + 1, 3), dtype=float)

        # Rodrigues 旋转公式生成同心、等半径的空间圆弧。
        for index in range(arc_segment_count + 1):
            angle = arc_angle * index / arc_segment_count
            rotated_radius = (
                start_unit * math.cos(angle)
                + np.cross(rotation_axis, start_unit) * math.sin(angle)
                + rotation_axis * np.dot(rotation_axis, start_unit) * (1.0 - math.cos(angle))
            )
            arc_points[index] = center + radius * rotated_radius

        arc_points[0] = first_tangent
        arc_points[-1] = second_tangent

        # 删除原轨迹的尖角部分，再接入两个切点之间的圆弧。
        first_cut_index = int(np.argmin(
            np.linalg.norm(xyz[:idx_tip + 1] - first_tangent, axis=1)
        ))
        second_cut_index = idx_tip + int(np.argmin(
            np.linalg.norm(xyz[idx_tip:] - second_tangent, axis=1)
        ))
        if second_cut_index <= first_cut_index:
            optimized.append(seg)
            continue

        pieces = []
        if first_cut_index > 0:
            pieces.append(xyz[:first_cut_index])
        pieces.append(arc_points)
        if second_cut_index + 1 < len(xyz):
            pieces.append(xyz[second_cut_index + 1:])
        final_xyz = np.vstack(pieces)

        # 清除拼接处可能出现的连续重复点。
        if len(final_xyz) >= 2:
            keep = np.r_[True, np.linalg.norm(np.diff(final_xyz, axis=0), axis=1) > 1e-9]
            final_xyz = final_xyz[keep]

        arc_middle_index = first_cut_index + len(arc_points) // 2
        phases = np.where(np.arange(len(final_xyz)) < arc_middle_index, 0, 1).astype(int)
        ...
        optimized.append(TrajectorySegment(
            ...
            note=(
                seg.note
                + f" [Fillet R={radius:.3f}, theta={math.degrees(theta):.3f}deg]"
            ),
            region=seg.region,
            phases=phases,
        ))
...
def main() -> None:
    ...
    # LOWER/UPPER 的 V 形尖角使用运动学约束内切圆弧过渡。
