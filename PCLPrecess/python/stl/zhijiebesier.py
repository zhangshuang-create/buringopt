...
DEFAULT_A_MAX = 1000.0
DEFAULT_J_MAX = 5000.0
EPS = 1.0e-9
...
def _sample_quintic_spline(
        points: np.ndarray,
        sample_count: int,
        smoothing: float,
) -> tuple[tuple, np.ndarray, float]:
    """Fit one spatial quintic spline and return samples plus max curvature."""
    ...
    keep = np.r_[True, deltas > 1.0e-8]
    pts = pts[keep]
    if len(pts) < 2:
        raise ValueError("Spline input is degenerate.")
    # splprep requires k+1 points.  Interpolate a short input to six points so
    # that every fitted trajectory still uses a genuine quintic basis.
    if len(pts) < 6:
        chord = np.linalg.norm(np.diff(pts, axis=0), axis=1)
        u0 = np.r_[0.0, np.cumsum(chord)]
        if u0[-1] <= EPS:
            raise ValueError("Spline input has zero length.")
        u0 /= u0[-1]
        u_dense = np.linspace(0.0, 1.0, 6)
        pts = np.column_stack([
            np.interp(u_dense, u0, pts[:, axis]) for axis in range(3)
        ])
    chord = np.linalg.norm(np.diff(pts, axis=0), axis=1)
    u = np.r_[0.0, np.cumsum(chord)]
    total = float(u[-1])
    if total <= EPS:
        raise ValueError("Spline input has zero length.")
    u /= total
    ...
    tck, _ = splprep(
        ...
        u=u,
        k=5,
        s=max(float(smoothing), 0.0),
        w=weights,
    )
    u_out = np.linspace(0.0, 1.0, max(8, int(sample_count)))
    xyz = np.column_stack(splev(u_out, tck))
    first = np.column_stack(splev(u_out, tck, der=1))
    second = np.column_stack(splev(u_out, tck, der=2))
    cross = np.cross(first, second)
    speed = np.linalg.norm(first, axis=1)
    curvature = np.linalg.norm(cross, axis=1) / np.maximum(speed, 1.0e-12) ** 3
    return tck, xyz, float(np.nanmax(curvature))


def fit_all_trajectory_quintic_curvature(
    ...
    """Fit every non-transition segment with a curvature-limited quintic spline.

    CENTER scans are fitted independently one by one.  LOWER/UPPER scans are
    already complete V-to-arc/fillet trajectories and are fitted as one piece.
    OVERTRAVEL and SEAM_BLEND_3D are motion connectors and remain untouched.
    """
    if speed <= 0.0 or a_max <= 0.0 or j_max <= 0.0:
        raise ValueError("speed, a_max, and j_max must be positive")
    radius_acc = speed ** 2 / a_max
    radius_jerk = math.sqrt(speed ** 3 / j_max)
    radius = max(radius_acc, radius_jerk)
    k_max = 1.0 / max(radius, EPS)
    step = max(float(sample_step), 1.0e-6)
    result: List[TrajectorySegment] = []
    transition_kinds = {"OVERTRAVEL", "SEAM_BLEND_3D"}

    for seg in trajectory:
        if seg.kind in transition_kinds or len(seg.xyz) < 2:
            result.append(seg)
            continue
        source = np.asarray(seg.xyz, dtype=float)
        source_d = np.linalg.norm(np.diff(source, axis=0), axis=1)
        source_len = float(np.sum(source_d))
        if source_len <= EPS:
            result.append(seg)
            continue
        count = max(8, int(math.ceil(source_len / step)) + 1)
        # Increase smoothing until the dense spline curvature is below the
        # dynamic limit.  The last fallback is a quintic smoothstep line,
        # whose curvature is identically zero and therefore always safe.
        smooth_values = [
            0.0,
            smooth_factor * len(source),
            4.0 * smooth_factor * len(source),
            16.0 * smooth_factor * len(source),
            64.0 * smooth_factor * len(source),
            256.0 * smooth_factor * len(source),
        ]
        fitted = None
        ...
        for smoothing in smooth_values:
            try:
                candidate_tck, candidate, max_curvature = _sample_quintic_spline(
                    source, count, smoothing
                )
            except Exception:
                continue
            if max_curvature < best_curvature:
                best_tck = candidate_tck
                best_candidate = candidate
                best_curvature = max_curvature
            if max_curvature <= k_max * 1.001:
                fitted = candidate
                break
        if fitted is None:
            if best_tck is None or best_candidate is None:
                u_line = np.linspace(0.0, 1.0, count)
                smoothstep = 10.0 * u_line ** 3 - 15.0 * u_line ** 4 + 6.0 * u_line ** 5
                fitted = source[0][None, :] + smoothstep[:, None] * (source[-1] - source[0])[None, :]
                max_curvature = 0.0
                fit_note = "quintic curvature-limited degenerate-line"
                best_candidate = fitted
            if fitted is not None:
                pass
            else:
                # A strict dynamic limit and fixed endpoints may make the original
                # shape infeasible.  Preserve the largest possible fraction of
                # the fitted quintic shape by blending it with its endpoint chord.
                # A line is itself a degree <= 5 spline, so this linear combination
                # remains a genuine quintic spline instead of falling back to a
                # different curve family.
                u_blend = np.linspace(0.0, 1.0, count)
                spline_xyz = np.column_stack(splev(u_blend, best_tck))
                spline_d1 = np.column_stack(splev(u_blend, best_tck, der=1))
                spline_d2 = np.column_stack(splev(u_blend, best_tck, der=2))
                chord_vec = source[-1] - source[0]
                line_xyz = source[0][None, :] + u_blend[:, None] * chord_vec[None, :]
                line_d1 = np.broadcast_to(chord_vec, spline_d1.shape)

                def blended(alpha: float) -> tuple[np.ndarray, float]:
                    xyz_alpha = line_xyz + alpha * (spline_xyz - line_xyz)
                    d1_alpha = line_d1 + alpha * (spline_d1 - line_d1)
                    d2_alpha = alpha * spline_d2
                    numerator = np.linalg.norm(np.cross(d1_alpha, d2_alpha), axis=1)
                    denominator = np.maximum(np.linalg.norm(d1_alpha, axis=1), 1.0e-12) ** 3
                    return xyz_alpha, float(np.nanmax(numerator / denominator))

                accepted_alpha = 0.0
                fitted, max_curvature = blended(0.0)
                # Search from the original shape toward the chord and retain the
                # first (largest) feasible shape fraction.
                for alpha in np.linspace(1.0, 0.0, 101):
                    candidate, candidate_k = blended(float(alpha))
                    if candidate_k <= k_max * 1.001:
                        accepted_alpha = float(alpha)
                        fitted = candidate
                        max_curvature = candidate_k
                        break
                fit_note = f"quintic curvature-limited blend={accepted_alpha:.3f}"
        else:
            fit_note = "quintic curvature-limited"
        fitted[0] = source[0]
        fitted[-1] = source[-1]

        fitted_uv = np.full((len(fitted), 2), np.nan, dtype=float)
        if len(seg.uv) == len(source) and np.isfinite(seg.uv).all():
            source_u = np.r_[0.0, np.cumsum(source_d)] / source_len
            target_u = np.linspace(0.0, 1.0, len(fitted))
            fitted_uv = np.column_stack([
                np.interp(target_u, source_u, seg.uv[:, axis])
                for axis in range(2)
            ])
        result.append(TrajectorySegment(
            kind=seg.kind,
            uv=fitted_uv,
            xyz=fitted,
            spray_on=seg.spray_on,
            note=(
                f"{seg.note} [{fit_note}; R={radius:.3f}mm; "
                f"kmax={k_max:.6g}; ksample={max_curvature:.6g}]"
            ),
            region=seg.region,
        ))
    return result
...
class TkControlPanel:
    def __init__(self, visualizer: InteractiveVisualizer):
        ...
        self.root.geometry("390x340+50+50")
        ...
        tk.Label(self.root, text="最大加速度 (mm/s²): ", font=("Arial", 11)).grid(
            row=3, column=0, padx=10, pady=6, sticky="w"
        )
        self.a_max_var = tk.StringVar(value=f"{getattr(self.visualizer, 'a_max', DEFAULT_A_MAX):.12g}")
        tk.Entry(self.root, textvariable=self.a_max_var, width=15, font=("Arial", 11)).grid(
            row=3, column=1, padx=10, pady=6, sticky="ew"
        )

        tk.Label(self.root, text="最大加加速度 (mm/s³): ", font=("Arial", 11)).grid(
            row=4, column=0, padx=10, pady=6, sticky="w"
        )
        self.j_max_var = tk.StringVar(value=f"{getattr(self.visualizer, 'j_max', DEFAULT_J_MAX):.12g}")
        tk.Entry(self.root, textvariable=self.j_max_var, width=15, font=("Arial", 11)).grid(
    ...
    def _read_control_values(self) -> tuple[float, float, float, float, float] | None:
        try:
            ...
            messagebox.showerror("错误", "间距、速度、最大加速度和最大加加速度必须大于 0。")
            return None
        return spacing, speed, spray_distance, a_max, j_max

    def _store_process_values(self, spacing: float, speed: float, spray_distance: float,
                              a_max: float, j_max: float) -> None:
    ...
    def update_spacing(self):
        ...
        if values is None:
            return
        new_d, speed, spray_distance, a_max, j_max = values
        self._store_process_values(new_d, speed, spray_distance, a_max, j_max)
        ...
        optimized_trajectory = fit_all_trajectory_quintic_curvature(
            optimized_trajectory,
            sample_step=self.visualizer.sample_step,
            speed=self.visualizer.trajectory_speed,
            a_max=getattr(self.visualizer, "a_max", DEFAULT_A_MAX),
            j_max=getattr(self.visualizer, "j_max", DEFAULT_J_MAX),
        )
    ...
    def export_trajectory(self):
        ...
        if values is None:
            return
        spacing, speed, spray_distance, a_max, j_max = values
        self._store_process_values(spacing, speed, spray_distance, a_max, j_max)
...
def _show_result(
    ...
    visualizer.a_max = float(a_max)
    visualizer.j_max = float(j_max)
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--a-max", type=float, default=DEFAULT_A_MAX,
                        help=f"Maximum acceleration in mm/s^2 (default: {DEFAULT_A_MAX}).")
    parser.add_argument("--j-max", type=float, default=DEFAULT_J_MAX,
                        help=f"Maximum jerk in mm/s^3 (default: {DEFAULT_J_MAX}).")
...
def main() -> None:
    ...
    if args.a_max <= 0.0 or args.j_max <= 0.0 or not np.isfinite(args.a_max) or not np.isfinite(args.j_max):
        raise ValueError("a-max and j-max must be finite and greater than zero.")
    ...
    final_3d_trajectory = fit_all_trajectory_quintic_curvature(
        final_3d_trajectory,
        sample_step=sample_step_val,
    ...
    if not args.no_show:
        _show_result(
            ...
            a_max=args.a_max, j_max=args.j_max,
