...
loft_uv_parameterization.py	基础版：完整管线 +厚度评估 + Tkinter UI
Proloft_uv.py	轨迹版：在基础版之上，新增斜向平行轨迹，去掉 argparse，主函数简化
Proloft_uv_parameterization.py	参数化版：重构了轨迹算法（腰部分区），移除厚度计算和 UI，CLI 参数最丰富
Dependencies:
...
def main():
    args = _parse_args()
    if args.ui or (not args.mesh and len(os.sys.argv) == 1):
        launch_parameter_ui()
        return
    ...
    run_pipeline(
        ...
        num_v=args.num_v,
        out_path=args.out,
        paq_out=args.paq_out,
        obj_path=args.obj,
        fig_prefix=args.fig_prefix,
        thickness_prefix=args.thickness_prefix,
        thickness_out=args.thickness_out,
        spray_distance=args.spray_distance,
        spray_sample_step=args.spray_sample_step,
        spray_sigma=args.spray_sigma,
        spray_peak=args.spray_peak,
        no_show=args.no_show,
        full_vis=args.full_vis,
        static_thickness_png=args.static_thickness_png,
    )
