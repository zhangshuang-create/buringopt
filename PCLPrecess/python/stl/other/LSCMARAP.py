...
For a mesh with exactly two open boundary loops, a multi-source Dijkstra solve
finds the globally shortest mesh-edge geodesic between the two boundaries.
Seam vertices are duplicated before parameterization so the annulus has a
disk-like topology suitable for texture mapping.
"""
...
def _shortest_boundary_geodesic(
    mesh: trimesh.Trimesh, first_loop: np.ndarray, second_loop: np.ndarray
) -> np.ndarray:
    """Return the globally shortest mesh-edge path between two boundaries."""
    vertices = np.asarray(mesh.vertices, dtype=float)
    edges = mesh_edges_from_faces(np.asarray(mesh.faces, dtype=np.int64))
    ...
    # A zero-weight sparse edge would be discarded, so use a negligible
    # positive connector from one super-source to every first-boundary vertex.
    super_source = len(vertices)
    positive = lengths[lengths > EPS]
    connector = (
        max(float(np.median(positive)), 1.0) * 1.0e-9
        if len(positive) else 1.0e-9
    )
    rows = np.concatenate([
        rows,
        np.full(len(first_loop), super_source, dtype=np.int64),
    ])
    columns = np.concatenate([columns, np.asarray(first_loop, dtype=np.int64)])
    weights = np.concatenate([
        weights,
        np.full(len(first_loop), connector, dtype=float),
    ])
    graph = coo_matrix(
    ...
    targets = np.asarray(second_loop, dtype=np.int64)
    target = int(targets[int(np.argmin(distance[targets]))])
    if not np.isfinite(distance[target]):
        raise ValueError("Could not find a geodesic path between the two boundaries")
    path = [target]
    while path[-1] != super_source:
        predecessor = int(predecessors[path[-1]])
        if predecessor < 0:
            raise ValueError("The boundary geodesic predecessor chain is incomplete")
        path.append(predecessor)
    path.reverse()
    return np.asarray(path[1:], dtype=np.int64)
...
def cut_two_boundary_geodesic(mesh: trimesh.Trimesh) -> CutMeshResult:
    faces = np.asarray(mesh.faces, dtype=np.int64)
    vertices = np.asarray(mesh.vertices, dtype=float)
    loops = ordered_boundary_loops(faces)
    if len(loops) != 2:
        raise ValueError(
            f"Boundary geodesic cutting requires exactly two boundaries, found {len(loops)}"
        )

    seam = _shortest_boundary_geodesic(mesh, loops[0], loops[1])
    ...
    if after_count != 1:
        raise ValueError(
            "The shortest boundary geodesic did not produce a disk-like mesh "
...
def prepare_parameterization_mesh(mesh: trimesh.Trimesh) -> CutMeshResult:
    ...
    if boundary_count == 2:
        return cut_two_boundary_geodesic(mesh)
    return CutMeshResult(
        mesh=mesh,
...
def main(argv: Optional[Sequence[str]] = None):
    ...
    if prepared.boundary_count_before == 2:
        seam_length = float(np.sum(np.linalg.norm(
            np.diff(prepared.seam_points, axis=0), axis=1
        )))
        print(
            "Two-boundary mesh detected: cut along the globally shortest "
            f"boundary geodesic ({len(prepared.seam_points)} seam vertices, "
            f"length {seam_length:.6g})."
        )
