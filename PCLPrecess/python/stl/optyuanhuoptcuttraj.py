        ...
        if on_iteration is not None and (
            ...
            for k, si in enumerate(center_idx):
                ...
                preview[si] = TrajectorySegment(
                    kind=orig.kind,
                    uv=np.full((len(lines[k]), 2), np.nan),
                    xyz=lines[k].copy(),
...
def _show_result(
    ...
    """Run interactive visualizer with live optimization display."""
    ...
    print("\n[实时优化完成！]")
    # 保存最终优化后的轨迹
    visualizer.current_trajectory = optimized_trajectory
