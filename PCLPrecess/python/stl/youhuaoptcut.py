...
def generate_seam_aware_trajectory(
    ...
    for index in range(paired_lower):
        ...
        merged_segment = TrajectorySegment(
            ...
            spray_on=True,
            note=f"{labels[0]} -> {labels[1]}",
            region="LOWER"  # 【关键修复】：补上区域标签，让下半区也进入 3D 圆弧后处理
        )
...
class InteractiveVisualizer:
    ...
    def update_trajectory(self, trajectory: Sequence[TrajectorySegment], planning_frame: PlanningFrame,
        ...
        self.chart_context_actor = add_penalty_chart(self.chart_renderer, candidates, objectives, opt_d, opt_f)
...
def _request_spacing(
    ...
    # ``vtktrajdisplay.request_spacing`` returns the accepted values for all
    # of its input fields.  Newer versions return ``(d, a_max, j_max)``
    # rather than just ``d``; this planner only needs the spacing value.
...
def main() -> None:
    ...
    _export_paq_trajectory(paq_path, mesh, optimized_trajectory,
    ...
    print(f"Trajectory events: {len(optimized_trajectory)}")
    if not args.no_show:
        _show_result(
            mesh, data, precut_seam, cut_edges, optimized_trajectory, planning_frame, metadata,
