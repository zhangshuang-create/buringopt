...
class UVBuildResult:
    ...
    waist_index: Optional[int] = None
    trajectory_spacing: Optional[float] = None
    lower_transition_count: Optional[int] = None
    upper_transition_count: Optional[int] = None
    gap_size: Optional[float] = None
    gap_bottom_v: Optional[float] = None
    gap_top_v: Optional[float] = None
    transition_uv: Optional[np.ndarray] = None
    transition_xyz: Optional[np.ndarray] = None
    seam_gap: Optional[float] = None
...
def _track_count_for_extent(left: float, right: float, spacing: float) -> int:
    """Number of u=kd tracks in a developed region, including u=0."""
    return 1 + int(math.floor((left + EPS) / spacing)) + int(math.floor((right + EPS) / spacing))


def _choose_shared_trajectory_spacing(
    target_spacing: float,
    lower_extent: Tuple[float, float],
    upper_extent: Tuple[float, float],
    seam_gap: Optional[float] = None,
) -> Tuple[float, int, int]:
    """
    Find the value nearest the requested spacing for which both halves have an
    even number of transitions after T0.  Consequently each half ends on an
    upward pass beside its start track and can continue into the other half.
    """
    target = max(float(target_spacing), EPS)
    lo = 0.5 * target
    hi = 1.5 * target
    candidates = np.linspace(lo, hi, 20001)
    candidates = np.unique(np.concatenate(([target], candidates)))
    best = None
    for spacing in candidates:
        # KEY CHANGE: l0/l1 are the same physical seam.  Plan only after
        # removing seam_gap/2 from each side of the developed UV domain.
        current_seam_gap = float(spacing) if seam_gap is None else float(seam_gap)
        half_gap = 0.5 * current_seam_gap
        lower_effective = (max(0.0, lower_extent[0] - half_gap), max(0.0, lower_extent[1] - half_gap))
        upper_effective = (max(0.0, upper_extent[0] - half_gap), max(0.0, upper_extent[1] - half_gap))
        lower_tracks = _track_count_for_extent(*lower_effective, float(spacing))
        upper_tracks = _track_count_for_extent(*upper_effective, float(spacing))
        lower_transitions = lower_tracks - 1
        upper_transitions = upper_tracks - 1
        if lower_tracks < 3 or upper_tracks < 3:
            continue
        if lower_transitions % 2 != 0 or upper_transitions % 2 != 0:
            continue
        score = abs(float(spacing) - target)
        if best is None or score < best[0]:
            best = (score, float(spacing), lower_transitions, upper_transitions)
    if best is None:
        raise ValueError(
            "Could not find a shared spacing with even upper/lower transition counts. "
            "Try a smaller --spacing value."
        )
    return best[1], best[2], best[3]


def _ordered_region_columns(valid_mask: np.ndarray, start_col: int, lower_half: bool) -> List[int]:
    available = [int(j) for j in range(valid_mask.shape[1]) if np.any(valid_mask[:, j])]
    if start_col not in available:
        start_col = min(available, key=lambda j: abs(j - start_col))
    if lower_half:
        # T0 -> left seam -> periodic jump to l1' -> return beside T0.
        return [start_col] + sorted((j for j in available if j < start_col), reverse=True) + sorted(
            (j for j in available if j > start_col), reverse=True
        )
    # Continue from the positive neighbour, move left, wrap at the seam, and
    # finish beside the upper start track.
    return [start_col] + sorted((j for j in available if j < start_col), reverse=True) + sorted(
        (j for j in available if j > start_col), reverse=True
    )


def _insert_exact_clip_rows(
    u_values: np.ndarray,
    v_values: np.ndarray,
    raw_xyz_grid: np.ndarray,
    left_boundary_u: np.ndarray,
    right_boundary_u: np.ndarray,
    inserted_v_values: Sequence[float],
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Insert exact scan/boundary intersections and return a freshly clipped grid."""
    entries = []
    all_v_values = list(map(float, v_values))
    for target_v in inserted_v_values:
        target_v = float(target_v)
        if 0.0 <= target_v <= 1.0 and not np.any(np.isclose(v_values, target_v, atol=1.0e-12)):
            all_v_values.append(target_v)
    all_v_values = sorted(set(all_v_values))

    for v_value in all_v_values:
        upper = int(np.searchsorted(v_values, v_value, side="right"))
        if upper == 0:
            xyz = raw_xyz_grid[0].copy()
        elif upper >= len(v_values):
            xyz = raw_xyz_grid[-1].copy()
        else:
            lower = upper - 1
            denom = float(v_values[upper] - v_values[lower])
            alpha = (v_value - float(v_values[lower])) / max(denom, EPS)
            xyz = (1.0 - alpha) * raw_xyz_grid[lower] + alpha * raw_xyz_grid[upper]
        left_u = float(np.interp(v_value, v_values, left_boundary_u))
        right_u = float(np.interp(v_value, v_values, right_boundary_u))
        valid = (u_values >= left_u - EPS) & (u_values <= right_u + EPS)
        uv = np.column_stack([u_values, np.full(len(u_values), v_value)])
        clipped_xyz = xyz.copy()
        clipped_uv = uv.copy()
        clipped_xyz[~valid] = np.nan
        clipped_uv[~valid] = np.nan
        entries.append((v_value, clipped_uv, clipped_xyz, valid, left_u, right_u, xyz))

    return (
        np.stack([item[1] for item in entries]),
        np.stack([item[2] for item in entries]),
        np.stack([item[3] for item in entries]),
        np.asarray([item[4] for item in entries]),
        np.asarray([item[5] for item in entries]),
        np.stack([item[6] for item in entries]),
    )


def _build_continuous_waist_zigzag_path(
    uv_grid: np.ndarray,
    xyz_grid: np.ndarray,
    valid_mask: np.ndarray,
    track_u_values: np.ndarray,
    left_boundary_u: np.ndarray,
    right_boundary_u: np.ndarray,
    raw_xyz_grid: np.ndarray,
    gap_bottom_v: float,
    gap_top_v: float,
    transition_start: Optional[Tuple[float, float]] = None,
    transition_end: Optional[Tuple[float, float]] = None,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Build two independently clipped snake paths plus exactly one transition."""
    rows, cols = valid_mask.shape
    zero_col = int(np.nanargmin(np.abs(np.nanmean(uv_grid[..., 0], axis=0))))
    row_v = np.nanmean(uv_grid[..., 1], axis=1)

    # KEY CHANGE: upperRegion and lowerRegion are disjoint.  No ordinary scan
    # line is allowed to survive inside (gap_bottom_v, gap_top_v).
    lower_mask = valid_mask.copy()
    lower_mask[row_v > gap_bottom_v + EPS, :] = False
    upper_mask = valid_mask.copy()
    upper_mask[row_v < gap_top_v - EPS, :] = False

    lower_order = _ordered_region_columns(lower_mask, zero_col, lower_half=True)
    if not lower_order:
        raise ValueError("The configured gap leaves no scan lines in the lower region.")

    uv_path: List[np.ndarray] = []
    xyz_path: List[np.ndarray] = []
    cell_path: List[Tuple[int, int]] = []

    def append_cell(r: int, c: int):
        if not valid_mask[r, c]:
            return
        if cell_path and cell_path[-1] == (r, c):
            return
        cell_path.append((r, c))
        uv_path.append(uv_grid[r, c])
        xyz_path.append(xyz_grid[r, c])

    def append_boundary_point(r: int, side: int):
        boundary_u = float(left_boundary_u[r] if side < 0 else right_boundary_u[r])
        xyz = np.array([
            np.interp(boundary_u, track_u_values, raw_xyz_grid[r, :, axis])
            for axis in range(3)
        ])
        marker_col = int(np.argmin(np.abs(track_u_values - boundary_u)))
        uv = np.array([boundary_u, float(row_v[r])])
        if uv_path and np.linalg.norm(uv_path[-1] - uv) < 1.0e-12:
            cell_path[-1] = (r, marker_col)
            return
        cell_path.append((r, marker_col))
        uv_path.append(uv)
        xyz_path.append(xyz)

    def append_connector(target_r: int, target_c: int):
        if not cell_path:
            append_cell(target_r, target_c)
            return
        source_r, source_c = cell_path[-1]
        source_u = float(uv_path[-1][0])
        target_u = float(uv_grid[target_r, target_c, 0])

        # l0 and l1 are the same physical seam.  Keeping the two end points in
        # sequence creates the requested explicit connector without traversing
        # all intermediate UV columns a second time.
        if source_u * target_u < 0.0 and abs(source_c - target_c) > 1:
            append_cell(target_r, target_c)
            return

        if source_r == target_r:
            step = 1 if target_c > source_c else -1
            for c in range(source_c + step, target_c + step, step):
                if valid_mask[source_r, c]:
                    append_cell(source_r, c)
            return

        # For tracks clipped by the hourglass waist, follow the corresponding
        # side boundary until the neighbouring track becomes valid.
        row_step = 1 if target_r > source_r else -1
        side = -1 if source_u < 0.0 or target_u < 0.0 else 1
        for r in range(source_r + row_step, target_r + row_step, row_step):
            append_boundary_point(r, side)
        append_cell(target_r, target_c)

    def append_track(column: int, region_mask: np.ndarray, upward: bool):
        track_rows = np.where(region_mask[:, column])[0]
        if len(track_rows) == 0:
            return
        ordered_rows = track_rows if upward else track_rows[::-1]
        append_connector(int(ordered_rows[0]), column)
        for r in ordered_rows:
            append_cell(int(r), column)

    # j0 is the bottom point of T0; the first movement is upward.
    for index, column in enumerate(lower_order):
        append_track(column, lower_mask, upward=(index % 2 == 0))

    # KEY CHANGE: finish lowerSegments first.  Do not call nearest-point
    # connection while switching regions.
    upper_available = [j for j in range(cols) if np.any(upper_mask[:, j])]
    if not upper_available:
        raise ValueError("The configured gap leaves no scan lines in the upper region.")
    last_col = cell_path[-1][1]
    upper_start = min(upper_available, key=lambda j: abs(j - last_col))
    upper_order = _ordered_region_columns(upper_mask, upper_start, lower_half=False)

    first_upper_rows = np.where(upper_mask[:, upper_order[0]])[0]
    first_upper_cell = (int(first_upper_rows[0]), int(upper_order[0]))
    automatic_start_uv = np.asarray(uv_path[-1], dtype=float)
    automatic_end_uv = np.asarray(uv_grid[first_upper_cell], dtype=float)

    # transitionStart/transitionEnd are configurable, but must coincide with
    # the two snake-path endpoints so finalPath stays continuous and no hidden
    # second connector is introduced.
    configured_start = automatic_start_uv if transition_start is None else np.asarray(transition_start, dtype=float)
    configured_end = automatic_end_uv if transition_end is None else np.asarray(transition_end, dtype=float)
    uv_tolerance = max(1.0e-6, 1.0e-6 * float(np.ptp(uv_grid[..., 0][np.isfinite(uv_grid[..., 0])])))
    if np.linalg.norm(configured_start - automatic_start_uv) > uv_tolerance:
        raise ValueError(f"transitionStart must equal the lower path endpoint {automatic_start_uv.tolist()}.")
    if np.linalg.norm(configured_end - automatic_end_uv) > uv_tolerance:
        raise ValueError(f"transitionEnd must equal the upper path start {automatic_end_uv.tolist()}.")

    # The only segment crossing the gap.  It is appended exactly once and uses
    # the same line representation/width as all other path segments.
    transition_uv = np.vstack([automatic_start_uv, automatic_end_uv])
    transition_xyz = np.vstack([xyz_path[-1], xyz_grid[first_upper_cell]])
    append_cell(*first_upper_cell)

    for index, column in enumerate(upper_order):
        append_track(column, upper_mask, upward=(index % 2 == 0))

    return np.vstack(uv_path), np.vstack(xyz_path), transition_uv, transition_xyz
...
def build_uv_grid(
    ...
    num_u: int,
    num_v: int,
    target_spacing: float = 40.0,
    gap_size: Optional[float] = None,
    seam_gap: Optional[float] = None,
    transition_start: Optional[Tuple[float, float]] = None,
    transition_end: Optional[Tuple[float, float]] = None,
) -> Tuple[np.ndarray, np.ndarray, UVBuildResult]:
    ...
    waist_index = int(np.argmin(lengths_arr))
    lower_extent = (
        float(np.max(left_lengths_arr[: waist_index + 1])),
        float(np.max(right_lengths_arr[: waist_index + 1])),
    )
    upper_extent = (
        float(np.max(left_lengths_arr[waist_index:])),
        float(np.max(right_lengths_arr[waist_index:])),
    )
    trajectory_spacing, lower_transition_count, upper_transition_count = _choose_shared_trajectory_spacing(
        target_spacing,
        lower_extent,
        upper_extent,
        seam_gap=seam_gap,
    )
    configured_seam_gap = trajectory_spacing if seam_gap is None else float(seam_gap)
    if configured_seam_gap <= 0.0:
        raise ValueError("seam_gap must be greater than zero.")
    half_seam_gap = 0.5 * configured_seam_gap
    planning_left_lengths = left_lengths_arr - half_seam_gap
    planning_right_lengths = right_lengths_arr - half_seam_gap
    if np.any(planning_left_lengths <= EPS) or np.any(planning_right_lengths <= EPS):
        raise ValueError("seam_gap is too large for the narrowest UV contour.")

    # KEY CHANGE: define a physical blank band d centered on the waist.  UV v
    # is normalized, so convert d through the sampled center-line arclength.
    configured_gap = trajectory_spacing if gap_size is None else float(gap_size)
    if configured_gap <= 0.0:
        raise ValueError("gap_size must be greater than zero.")
    contour_centers = np.vstack([np.mean(info[0], axis=0) for info in contour_infos])
    center_steps = np.linalg.norm(np.diff(contour_centers, axis=0), axis=1)
    center_arc = np.concatenate([[0.0], np.cumsum(center_steps)])
    total_height = float(center_arc[-1])
    if total_height < EPS:
        raise ValueError("Cannot create a physical gap on a zero-height surface.")
    if configured_gap >= total_height:
        raise ValueError("gap_size must be smaller than the surface height.")
    waist_height = float(center_arc[waist_index])
    gap_bottom_height = min(max(waist_height - 0.5 * configured_gap, 0.0), total_height - configured_gap)
    gap_top_height = gap_bottom_height + configured_gap
    gap_bottom_v = float(np.interp(gap_bottom_height, center_arc, v_values))
    gap_top_v = float(np.interp(gap_top_height, center_arc, v_values))
    max_left_steps = int(math.floor((float(np.max(planning_left_lengths)) + EPS) / trajectory_spacing))
    max_right_steps = int(math.floor((float(np.max(planning_right_lengths)) + EPS) / trajectory_spacing))
    overlay_u_values = trajectory_spacing * np.arange(-max_left_steps, max_right_steps + 1, dtype=float)
    overlay_uv_layers = []
    overlay_xyz_layers = []
    overlay_raw_xyz_layers = []
    overlay_valid_layers = []
    for i, (v_value, (contour, start_point, opposite_point)) in enumerate(zip(v_values, contour_infos)):
        valid = (overlay_u_values >= -planning_left_lengths[i] - EPS) & (overlay_u_values <= planning_right_lengths[i] + EPS)
        ...
        overlay_raw_xyz_layers.append(sampled.copy())
        overlay_uv = np.column_stack([overlay_u_values, np.full(len(overlay_u_values), v_value)])
    ...
    overlay_raw_xyz_grid = np.stack(overlay_raw_xyz_layers, axis=0)
    overlay_valid_mask = np.stack(overlay_valid_layers, axis=0)

    # Insert the exact v-value where each vertical u=constant scan line meets
    # the smooth left/right UV boundary.  This removes grid-row stair stepping.
    left_boundary_profile = -planning_left_lengths
    right_boundary_profile = planning_right_lengths
    clip_intersections = [gap_bottom_v, gap_top_v]
    for u_value in overlay_u_values:
        boundary_profile = left_boundary_profile if u_value < 0.0 else right_boundary_profile
        difference = boundary_profile - float(u_value)
        for i in range(len(v_values) - 1):
            a = float(difference[i])
            b = float(difference[i + 1])
            if a == 0.0:
                clip_intersections.append(float(v_values[i]))
            elif a * b < 0.0:
                ratio = abs(a) / (abs(a) + abs(b))
                clip_intersections.append(float((1.0 - ratio) * v_values[i] + ratio * v_values[i + 1]))

    (
        overlay_uv_grid,
        overlay_xyz_grid,
        overlay_valid_mask,
        exact_left_boundary_u,
        exact_right_boundary_u,
        overlay_raw_xyz_grid,
    ) = _insert_exact_clip_rows(
        overlay_u_values,
        v_values,
        overlay_raw_xyz_grid,
    ...
    zigzag_uv, zigzag_xyz, transition_uv, transition_xyz = _build_continuous_waist_zigzag_path(
        overlay_uv_grid,
        overlay_xyz_grid,
        overlay_valid_mask,
        overlay_u_values,
    ...
    result = UVBuildResult(
        ...
        trajectory_spacing=trajectory_spacing,
        lower_transition_count=lower_transition_count,
        upper_transition_count=upper_transition_count,
        gap_size=configured_gap,
        gap_bottom_v=gap_bottom_v,
        gap_top_v=gap_top_v,
        transition_uv=transition_uv,
        transition_xyz=transition_xyz,
        seam_gap=configured_seam_gap,
    )
...
def _break_periodic_uv_jumps(points: np.ndarray, max_u_jump: float) -> np.ndarray:
    """Add display-only breaks at UV seam jumps; the 3D/TXT path stays continuous."""
    pts = np.asarray(points, dtype=float)
    if len(pts) < 2:
        return pts.copy()
    output = [pts[0]]
    for previous, current in zip(pts[:-1], pts[1:]):
        if (
            np.isfinite(previous).all()
            and np.isfinite(current).all()
            and abs(float(current[0] - previous[0])) > max_u_jump
        ):
            output.append(np.array([np.nan, np.nan]))
...
def visualize_uv_grid(
    ...
    fig = plt.figure(figsize=(15, 5) if full_vis else (12, 5))
    max_faces = None if full_vis else 800
    max_u_lines = None if full_vis else 24
    max_v_lines = None if full_vis else 32
    max_scatter = None if full_vis else 2500

    ax1 = fig.add_subplot(131 if full_vis else 121, projection="3d")
    ...
    if result is not None:
        ...
        if full_vis and result.start_reference_curve is not None:
            ref = result.start_reference_curve
            ax1.plot(ref[:, 0], ref[:, 1], ref[:, 2], color="#ff00ff", linewidth=3.0, label="u=0 base")
        if full_vis and result.center_sweep_curve is not None:
    ...
    ax1.set_title("3D trajectory" if not full_vis else "3D UV grid")
    ax1.legend(loc="best")
    _set_axes_equal(ax1)

    ax2 = fig.add_subplot(132 if full_vis else 122)

    # The original UV parameterization is a closed domain.  Build its polygon
    # from the left boundary bottom->top and the right boundary top->bottom.
    # Connecting uv_grid[:, j] would be incorrect: a fixed array index j is
    # not a constant-u line when successive contours have different lengths.
    original_left = []
    original_right = []
    for row in uv_grid:
        finite = row[np.isfinite(row).all(axis=1)]
        if len(finite):
            original_left.append(finite[np.argmin(finite[:, 0])])
            original_right.append(finite[np.argmax(finite[:, 0])])
    if original_left:
        original_left_arr = np.asarray(original_left)
        original_right_arr = np.asarray(original_right)
        original_domain = np.vstack([original_left_arr, original_right_arr[::-1], original_left_arr[:1]])
        ax2.fill(
            original_domain[:, 0],
            original_domain[:, 1],
            facecolor="#80c7ff",
            ...
            linewidth=2.0,
            alpha=0.34,
            label="original UV domain",
            zorder=0,
        )

    if full_vis:
        # Horizontal contour levels are valid UV grid lines.
        for i in range(uv_grid.shape[0]):
            _plot_valid_2d_line(ax2, uv_grid[i], color="#1f77b4", linewidth=0.8)
    if full_vis and result is not None and result.overlay_uv_grid is not None:
    ...
    if result is not None and result.zigzag_uv is not None and len(result.zigzag_uv) >= 2:
        display_trajectory_uv = result.zigzag_uv
        if result.trajectory_spacing is not None:
            display_trajectory_uv = _break_periodic_uv_jumps(
                display_trajectory_uv,
                max_u_jump=3.0 * result.trajectory_spacing,
            )
        _plot_valid_2d_line(ax2, display_trajectory_uv, color="#111111", linewidth=1.15)
        ax2.plot([], [], color="#111111", linewidth=1.3, label="trajectory")
    ...
    # Default mode stops here: the two additional 3D colour-map views were the
    # main source of slow window creation and rotation.  They remain available
    # explicitly through --full-vis.
    if not full_vis:
        fig.tight_layout()
        if prefix:
            fig.savefig(prefix.with_name(prefix.name + "_overview.png"), dpi=150)
        if show:
            plt.show()
        else:
            plt.close(fig)
...
def run_pipeline(
    ...
    uv_grid, xyz_grid, result = build_uv_grid(
        mesh,
        num_u,
        num_v,
    ...
    print(f"Saved the only output file: {trajectory_out} ({paq_count} trajectory points)")
    print(f"Waist layer: {result.waist_index}/{num_u - 1}")
    print(f"Requested spacing: {target_spacing:.6g}")
    print(f"Optimized shared spacing: {result.trajectory_spacing:.6g}")
    print(f"Waist gap d: {result.gap_size:.6g}")
    print(f"Periodic seam gap: {result.seam_gap:.6g} ({0.5 * result.seam_gap:.6g} inset per UV side)")
    print(f"Gap UV interval: [{result.gap_bottom_v:.6g}, {result.gap_top_v:.6g}]")
    print(f"transitionStart: {result.transition_uv[0]}")
    print(f"transitionEnd: {result.transition_uv[1]}")
    print(f"Lower even transition count: {result.lower_transition_count}")
    print(f"Upper even transition count: {result.upper_transition_count}")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate one continuous trajectory on a loft surface.")
    parser.add_argument("mesh", nargs="?", default="", help="Optional STL/OBJ/PLY path; otherwise a file dialog opens.")
    parser.add_argument("--spacing", type=float, default=15.0, help="Requested trajectory spacing (default: 15).")
    parser.add_argument("--gap", type=float, default=None, help="Physical waist gap d; defaults to the optimized spacing.")
    parser.add_argument("--seam-gap", type=float, default=None, help="Total periodic seam clearance; defaults to spacing d.")
    parser.add_argument("--transition-start", nargs=2, type=float, metavar=("U", "V"), help="Explicit transitionStart in UV.")
    parser.add_argument("--transition-end", nargs=2, type=float, metavar=("U", "V"), help="Explicit transitionEnd in UV.")
    parser.add_argument("--num-u", type=int, default=101, help="Number of horizontal sampling layers.")
    parser.add_argument("--num-v", type=int, default=96, help="Number of arclength samples on each layer.")
    parser.add_argument("--out", default="", help="Trajectory TXT path; defaults beside the selected mesh.")
    parser.add_argument("--no-show", action="store_true", help="Generate the TXT without opening visualization.")
...
def main():
    args = _parse_args()
    mesh_path = args.mesh or select_mesh_file_dialog()
    if not mesh_path:
        print("No mesh file selected; nothing was generated.")
        return
    mesh_file = Path(mesh_path)
    trajectory_out = args.out or str(mesh_file.with_name(mesh_file.stem + "_trajectory.txt"))
    run_pipeline(
        mesh_path=mesh_path,
        num_u=args.num_u,
        num_v=args.num_v,
        ...
        transition_start=tuple(args.transition_start) if args.transition_start else None,
        transition_end=tuple(args.transition_end) if args.transition_end else None,
        no_show=args.no_show,
        full_vis=args.full_vis,
    )
