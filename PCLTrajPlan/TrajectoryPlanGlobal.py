...
def main() -> None:
    ...
    print("Standard Isomap: sampling the original STL surface directly.")
    rng = np.random.default_rng(args.seed)
    requested_count = max(3, int(round(float(mesh.area) * args.density)))
    # Dense boundary oversampling is disabled because it makes the point-cloud
    # kNN graph density-biased near the boundary.
    ...
    print("Boundary spacing multiplier: 0x")
    ...
    print("Inverse mapping backend: point-cloud local affine regression")
